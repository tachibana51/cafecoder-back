...
func testcaseHandler(w http.ResponseWriter, r *http.Request, sqlCon **cafedb.MyCon) {
	...
	case "GET":
		...
		//read results from db
		rows, err := (*sqlCon).SafeSelect("SELECT DISTINCT testcase_results.name, testcase_results.result, testcase_results.time FROM testcase_results WHERE testcase_results.session_id='%s' ORDER BY testcase_results.name", jsonData.CodeSession)
		defer rows.Close()
		if err != nil {
			fmt.Println(err)
			return
