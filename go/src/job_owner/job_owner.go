...
func initFromDB(toJobQueue *mutexJobQueue, sqlCon **cafedb.MyCon) {
	rows, err := (*sqlCon).SafeSelect("SELECT code_sessions.id FROM code_sessions WHERE code_sessions.result='WJ' OR code_sessions.result='IE'")
	if err != nil {
		fmt.Println(err)
	}
	var bufStr string
...
func doFromJudgeThread(con net.Conn, jobMap *mutexJobMap, toJobQueue *mutexJobQueue, sqlCon **cafedb.MyCon) {
	...
	_, err := (*sqlCon).PrepareExec("UPDATE code_sessions SET result=? , error=LEFT(?, 4095) WHERE code_sessions.id=? AND (code_sessions.result='WJ' OR code_sessions.result='IE')", result, jsonResult.ErrMessage, codeSession)
	if err != nil {
		fmt.Println(err)
	}
	for _, testcase := range jsonResult.Testcases {
		...
		_, err = (*sqlCon).PrepareExec("INSERT INTO testcase_results (id, session_id, name, result, time) VALUES(?, ?, ?, ?, ?)", id, codeSession, caseName, caseResult, caseTime)
		if err != nil {
			fmt.Println(err)
		}
	}
